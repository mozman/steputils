...
from typing import Iterable, Union, Tuple, List, Optional, TextIO, Sequence
...
__all__ = [
    ...
    "is_parameter_list",
    "is_entity",
    "is_simple_entity_instance",
    "is_complex_entity_instance",
    "keyword",
    "reference",
    "enum",
    "binary",
...
)  # should accept ISO-10303-21 and lower case letters
...
class ParameterList(tuple):
    ...
    # list: (arg1, list2, ...)
    #
    # The elements of aggregates (SET, BAG, LIST, ARRAY) are given in parentheses,
    # separated by ",".
    def __str__(self):
...
class TypedParameter:
    """Typed parameter, `type_name` is the type of the parameter, `param` is the
    parameter itself.
    """
...
class Entity:
    """STEP-file entity, `name` is the type of the entity, `params` are the entity
    parameters as a :class:`ParameterList`.
    """
...
class SimpleEntityInstance(EntityInstance):
    """Simple instance entity, `ref` is the instance name as string (e.g. ``'#100'``),
    `entity` is the :class:`Entity` object.
    """
...
class ComplexEntityInstance(EntityInstance):
    """A complex entity instance consist of multiple :class:`Entity` objects, `ref` is
    the instance name as string (e.g. ``'#100'``)
    """
...
class HeaderSection:
    """The HEADER section has a fixed structure consisting of 3 to 6 groups in the given
    order. Except for the data fields time_stamp and FILE_SCHEMA all fields may contain
    empty strings.

    """

    REQUIRED_HEADER_ENTITIES = ("FILE_DESCRIPTION", "FILE_NAME", "FILE_SCHEMA")
    OPTIONAL_HEADER_ENTITIES = (
        "FILE_POPULATION",
        "SECTION_LANGUAGE",
        "SECTION_CONTENT",
    )
    KNOWN_HEADER_ENTITIES = set(REQUIRED_HEADER_ENTITIES) | set(
        OPTIONAL_HEADER_ENTITIES
    )
    ...
    def set_file_description(
        self, description: Optional[Sequence] = None, level: str = "2;1"
    ) -> None:
        description = ParameterList(description) if description else ParameterList()
        self.add(
            Entity(
                "FILE_DESCRIPTION",
                ParameterList((ParameterList(description), str(level))),
            )
        )
    ...
    def set_file_name(
        ...
        if isinstance(organization, str):
            ...
            organization = ParameterList(organization)
    ...
    def write(self, fp: TextIO) -> None:
        def write_entities(names, optional=False):
            for name in names:
                try:
                    ...
                    if not optional:
                        raise StepFileStructureError(
                            f"Missing required header entity: {name}"
                        )
                else:
                    fp.write(str(entity))
        ...
        unknown_header_entities = (
            set(self.entities.keys()) - HeaderSection.KNOWN_HEADER_ENTITIES
        )
        if len(unknown_header_entities):
            raise StepFileStructureError(
                f"Found unsupported header entities: {unknown_header_entities}"
            )
...
class DataSection:
    ...
    The encoding of this data follows some simple principles.

    The attribute `name`, if not `None`, should be a unique data section name.
    The attribute `schema` defines the schema that shall govern this data section.
    The schema name must appear in the header section `file_schema` entry. If a `name`
    is set, a valid `schema` has to be set also.
    ...
    def add(self, instance: EntityInstance) -> None:
        """Append new entity `instance`. Replaces existing instances with same instance
...
class StepFile:
    ...
    def __init__(self):
        ...
        self._linked_data_sections: Optional[ChainMap] = None

    def __getitem__(self, ref: str):
        """Returns :class:`EntityInstance` by instance name `ref`. Searches all data
        sections if more than one exist.
    ...
    def get(self, ref: Reference) -> Optional[EntityInstance]:
        """Returns :class:`EntityInstance` by instance name `ref` or ``None`` if not
        found. Searches all data sections if more than one exist.
    ...
    def new_data_section(
        self, name: Optional[str] = None, schema: Optional[str] = None
    ) -> DataSection:
    ...
    def write(self, fp: TextIO) -> None:
        """
        Serialize to a STEP-file (ISO 10303-21) formatted stream to ``fp`` (a
        :meth:`write`-supporting file-like object).

        File encoding should be ``'iso-8859-1'`` but can also be ``'ascii'``, because
        ISO 10303-21 requires special encoding for characters > 126 into characters <
        127 as unicode compatible characters, which should be compatible with most
    ...
    def __str__(self) -> str:
        ...
        Special encoding for characters > 126 into characters < 127 as unicode compatible
        characters according to ISO 10303-21 standard will be applied.
...
class Lexer:
    ...
    def parse(self) -> Iterable[str]:
        def check_valid_terminator():
            c = self.buffer.look()
            if (
                c not in PARAM_TERMINATOR and c > " "
            ):  # space, \t or \n also terminates parameters
                raise ParseError(
                    f'Expected parameter terminator "," or ")" in line {self.buffer.line_number}.'
                )
        ...
        while current != EOF:
            if current <= " ":
                ...
                raise ParseError(
                    f"Unexpected character {current} in line {self.buffer.line_number}."
                )
            current = self.buffer.look()
    ...
    def enum(self):
        ...
        while True:
            ...
            if current in ENUM_CHARS:
                ...
                estr = "".join(s)
                raise ParseError(
                    f'Found invalid enum "{estr}" in line {b.line_number}.'
                )
...
class Parser:
    ...
    def _keyword(self):
        ...
        if KEYWORD.fullmatch(self.lookahead):
            ...
            raise ParseError(
                f'Invalid keyword "{self.lookahead}" in instance: {self.current_instance}.'
            )
    ...
    def _typed_param(self) -> TypedParameter:
        ...
        if self.pop() != ")":
            raise ParseError(
                f'Expected ")" after typed parameter "{name}" in instance: {self.current_instance}.'
            )
    ...
    def _parameter_list(self) -> ParameterList:
        if self.pop() != "(":
            raise ParseError(
                f'Expected "(" for parameter list in instance: {self.current_instance}.'
            )
    ...
    def _instance(self) -> EntityInstance:
        ...
        if self.lookahead == "(":  # Complex Instance Entity
            ...
            if self.pop() != ";":
                raise ParseError(
                    f'Missing ";" after complex entity instance: {instance_id}'
                )
            ...
            if self.pop() != ";":
                raise ParseError(
                    f'Missing ";" after simple entity instance: {instance_id}.'
                )
    ...
    def _data_section(self) -> DataSection:
        ...
        if isinstance(self.lookahead, ParameterList):
            ...
            if (
                len(params) == 2
                and is_string(params[0])
                and is_parameter_list(params[1])
            ):
                name = params[0]
                ...
                if len(schema_list) == 1:
                    ...
                    raise ParseError(
                        "Expected schema parameter as single string in a list ('SCHEMA')."
                    )
            else:
                raise ParseError(
                    "Expected data section parameters DATA('SECTION_NAME',('SCHEMA'))."
                )
        else:
            if self.pop() != ";":
...
def keyword(name: str) -> Keyword:
    """Factory function to create a new :class:`Keyword` object. Only uppercase letters
    and digits are allowed, standard keyword has to start with an uppercase letter and
    user defined keyword has to start with ``'!'``.
    """
...
def reference(ref: str) -> Reference:
    """Factory function to create a new :class:`Reference` object (Entity Instance Name).
    A reference has to start with ``'#'`` followed by only digits e.g. ``'#100'``
    """
...
def enum(enum: str) -> Enumeration:
    """Factory function to create a new :class:`Enumeration` object. An enumeration is
    surrounded ``'.'`` and only uppercase letters and digits are allowed
    e.g. ``'.TRUE.'`` or ``'.FALSE.'``.
    """
...
def binary(value: int, unset: int = 0) -> Binary:
    """Factory function to create a new :class:`Binary` object. Only for export used,
    `unset` specifies the uppermost unset bits.
    """
...
def unset_parameter(char: str) -> UnsetParameter:
    """Factory function to create a new :class:`UnsetParameter` object. Unset attribute
    values are given as ``'$'``. Explicit attributes which got re-declared as derived in
    a subtype are encoded as ``'*'`` in the position of the supertype attribute.
    """
    if char not in "*$":
...
def simple_instance(ref: str, name: str, params: AnyList) -> SimpleEntityInstance:
    """Factory function to create a new :class:`SimpleEntityInstance` object.
    This method creates the :class:`Entity` object automatically.
...
def loads(s: str) -> StepFile:
    ...
    Decoding for special characters > 126 to unicode characters according to ISO
    10303-21:2002 standard will be applied.
...
def load(fp: TextIO) -> StepFile:
    ...
    A special encoding form characters > 126 is applied in STEP-Files, therefore an
    encoding setting at opening files is not necessary, reading as ``'ascii'`` works
    fine. Decoding of this special characters will be applied.
    ISO 10303-21:2016 files are not supported yet, they are ``'UTF-8'`` encoded, this
    encoding can be used also for older STEP-files, because code points < 127 are equal
    to ``'ISO-8859-1'`` or default ``'ascii'`` encoding.
